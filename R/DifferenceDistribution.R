...
#' both the \code{A} and \code{B} processes have the same success rate.  When \code{nA} and \code{nB} don't divide evenly into to each
...
#' (no padding needed),  or  \code{nA >> nB}, or \code{nB >> nA} (padding is small effect).
#'
